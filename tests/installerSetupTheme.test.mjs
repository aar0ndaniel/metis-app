...
for (const [name, source] of [
  ...
  assert.match(source, /function getInitialSetupTheme\(\): SetupTheme \{\s+return 'Light'\s+\}/, `${name} should always open with Light selected by default.`)
  ...
  assert.match(source, /document\.documentElement\.setAttribute\('data-theme', theme === 'Light' \? 'light' : 'dark'\)/, `${name} should preview the selected theme immediately.`)
  assert.match(source, /const FF = 'Matter, sans-serif'/, `${name} should use the local Matter font instead of Inter.`)
  assert.match(source, /Appearance\s*<\/span>/, `${name} should expose the simplified Appearance section from the Pencil mockup.`)
  assert.match(source, /const Icon = option === 'Light' \? Sun : Moon/, `${name} should use Phosphor icons in the appearance control.`)
  assert.match(source, /<Icon size=\{12\}/, `${name} should render the Phosphor icon in the appearance control.`)
  assert.doesNotMatch(source, /function ThemePreviewMockup/, `${name} should not render the old large theme mockups.`)
  assert.match(source, /overflow: 'hidden'[\s\S]{0,420}<img[\s\S]{0,420}maxWidth:/, `${name} should clip and size the app logo inside its frame.`)
...
assert.match(setupWizard, /<Copy size=\{11\}/, 'Setup wizard should keep the package snippet copy action.')
...
assert.match(setupWizard, /const renderRPaused = \(\) => \([\s\S]*<div style=\{\{ display: 'flex', alignItems: 'center', gap: 12, borderRadius: 12, background: 'transparent', border: 'none', padding: '14px 16px' \}\}>[\s\S]{0,900}Need R\?/, 'Setup wizard Need R helper row should not render an outer filled card or border.')
assert.match(setupWizard, /const renderInstalling = \(\) => \([\s\S]*background:\s*'transparent'[\s\S]*border:\s*'none'[\s\S]*\{activeStep\.label\}/, 'Setup wizard loading panel should show progress without a filled card or border.')
...
assert.match(installerPreview, /className="installer-preview-phase-content"[\s\S]*overflow:\s*'visible'/, 'Installer preview phase content should stay visible inside the taller shell.')
assert.match(electronMain, /const installerPreviewWidth = 400/, 'Installer windows should use the approved 400px width.')
assert.match(electronMain, /const installerPreviewHeight = 500/, 'Installer windows should use the approved 500px height.')
