...
assert.match(source, /normalizeWorkspaceAccentColor/, 'Workspace Home should normalize legacy workspace colors to the current accent.')
assert.match(source, /getWorkspaceAccentPalette/, 'Workspace Home color menus should include the current accent.')
assert.match(source, /id="tour-new-workspace"[\s\S]*backgroundColor:\s*'transparent'[\s\S]*<PlusCircle size=\{15\} color="var\(--color-accent\)"/, 'New workspace plus button should have no filled background and use an accent-colored icon.')
assert.match(source, /id="tour-new-model"[\s\S]*backgroundColor:\s*'var\(--color-accent\)'[\s\S]*<Plus size=\{14\} weight="bold" color="var\(--color-on-accent\)"/, 'New model CTA should use the current accent background and on-accent icon.')
assert.match(newWorkspaceSource, /getActiveAccentColor/, 'New workspace dialog should default to the current preference accent.')
assert.match(newWorkspaceSource, /backgroundColor:\s*'var\(--color-accent\)'/, 'New workspace create button should use the current preference accent.')
assert.match(newModelSource, /getActiveAccentColor/, 'New model dialog should use the current preference accent for inline workspace creation.')
