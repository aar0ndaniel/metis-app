...
assert.match(appSource, /action === 'open-workspace'[\s\S]*openFile\?\.\([\s\S]*extensions:\s*\['metisws'\][\s\S]*openWorkspaceFromFilePath/, 'Open Workspace should select only .metisws files, not a folder or legacy extension.')
assert.doesNotMatch(appSource, /\.ada|['"]ada['"]/, 'Renderer workspace flows should not retain the legacy .ada extension.')
...
assert.doesNotMatch(electronMainSource, /LEGACY_WORKSPACE_FILE_EXTENSION|\.ada|AdaFile|adaFilePath|readLegacyWorkspaceFolder/, 'Electron should not retain legacy workspace extension or format support.')
assert.match(electronMainSource, /isWorkspacePathAllowed\(workspacePath\)/, 'Dataset persistence should accept an approved .metisws workspace file opened by the user.')
