...
const start = source.indexOf('normalize_plspredict_cv_result <- function') !== -1
  ? source.indexOf('normalize_plspredict_cv_result <- function')
  : source.indexOf('extract_plspredict_sections <- function')
...
assert.match(source, /calculate_plspredict_q2|sse_pls|SSE_PLS|SSE_NAIVE/i, 'Q²predict should use the fold training-mean benchmark.')
assert.match(source, /PLS[-_]SEM[-_]RMSE|PLS_SEM_RMSE/, 'The backend should preserve the exact PLSpredict metric keys.')
...
assert.match(extractor, /run_cvpat_assessment\(\s*prediction_core,/, 'HOC CVPAT must use the same repeated-indicator prediction core as PLSpredict.')
