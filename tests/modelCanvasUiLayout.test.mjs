...
assert.match(titleBar, /height:\s*36,[\s\S]*padding:\s*isMac \? \(isFullScreen \? '0 16px' : '0 16px 0 80px'\) : '0 0 0 16px'/, 'title bar should keep its compact height, reserve traffic light space on macOS, and keep Windows controls flush right')
...
assert.match(source, /onClick=\{\(\) => updateSelected\(\{ shape: 'rectangle' \}\)\}/, 'construct shape selector should offer Rectangle.')
assert.match(source, /normalizeConstructShape\(shape\?: ConstructShape\): 'circle' \| 'oval' \| 'rectangle'[\s\S]*shape === 'rectangle' \? 'rectangle'[\s\S]*shape === 'oval' \|\| shape === 'square' \? 'oval' : 'circle'/, 'legacy saved square constructs should render as ovals while rectangle stays distinct.')
...
assert.match(source, /isRectangle[\s\S]*<rect[\s\S]*x=\{-constructRadii\.rx\}[\s\S]*width=\{constructRadii\.rx \* 2\}[\s\S]*height=\{constructRadii\.ry \* 2\}/, 'rectangle constructs should render as rectangles using the oval width and height dimensions.')
...
assert.match(source, /hasIndependentDimensions[\s\S]*h: 'left' as ResizeHandle[\s\S]*h: 'right' as ResizeHandle[\s\S]*h: 'top' as ResizeHandle[\s\S]*h: 'bottom' as ResizeHandle/, 'selected oval and rectangle should render left, right, top, and bottom resize handles')
...
assert.match(source, /const STRUCTURAL_PATH_STROKE_WIDTH = 2\.4/, 'structural paths should use a thicker named stroke width')
assert.match(source, /const SELECTED_PATH_STROKE_WIDTH = 3\.2/, 'selected structural paths should use a thicker named stroke width')
assert.match(source, /const INDICATOR_PATH_STROKE_WIDTH = 1\.8/, 'indicator and HOC-to-LOC paths should use a thicker named stroke width')
assert.match(source, /strokeWidth=\{isPathSel \? SELECTED_PATH_STROKE_WIDTH : STRUCTURAL_PATH_STROKE_WIDTH\}/, 'structural path rendering should use the named stroke widths')
assert.match(source, /strokeWidth=\{INDICATOR_PATH_STROKE_WIDTH\}/, 'indicator and HOC-to-LOC path rendering should use the named stroke width')
