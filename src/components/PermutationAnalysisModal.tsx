...
export interface PermutationAnalysisSettings {
...
export type PermutationConfiguralStatus = 'idle' | 'checking' | 'passed' | 'failed'

export interface PermutationAnalysisModalProps {
  modelName: string
  ...
  isRunning?: boolean
  onClose: () => void
  onPrecheck?: (settings: PermutationAnalysisSettings) => Promise<any> | any
  onRun?: (settings: PermutationAnalysisSettings) => void
}
...
const DEFAULT_PERMUTATIONS = 500
...
function readPrecheckPassed(result: any): boolean {
  const value =
    result?.results?.configuralInvariance?.passed ??
    result?.configuralInvariance?.passed ??
...
export default function PermutationAnalysisModal({
  ...
  groupingOptions,
  datasetRows = [],
  configuralStatus: configuralStatusProp,
  isRunning = false,
  onClose,
  onPrecheck,
  onRun,
}: PermutationAnalysisModalProps) {
  ...
  const excludedMissing = groupSummary.excludedMissing
  ...
  const calculateDisabled =
    ...
    !groupingVariable ||
    !hasExactlyTwoGroups ||
    !configuralPrecheckPassed ||
    Boolean(permutationsError || alphaError || seedError)

  const buildSettings = (): PermutationAnalysisSettings => ({
    groupingVariable,
    groupA,
    groupB,
    permutations: Number(permutationsInput),
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Permutation Analysis"
      ...
      >
        ...
        >
          ...
          <button
            type="button"
            aria-label="Close permutation analysis"
        ...
        <div className="flex-1" style={{ minHeight: 0, overflowY: configuralOpen ? 'auto' : 'hidden', padding: '18px 32px 14px' }}>
          <div className="grid" style={{ gridTemplateColumns: '120px minmax(0, 1fr)', gap: '10px 16px', alignItems: 'center' }}>
            <label htmlFor="permutation-grouping-variable-button" style={labelStyle}>
              Grouping variable
            </label>
            <div className="relative">
              <button
                id="permutation-grouping-variable-button"
                type="button"
                aria-haspopup="listbox"
                aria-expanded={groupingDropdownOpen}
                aria-describedby={groupingCountMessage ? 'permutation-grouping-count-message' : undefined}
                disabled={isRunning}
              ...
              {groupingCountMessage && (
                <div
                  id="permutation-grouping-count-message"
                  role="alert"
                  aria-live="polite"
                  style={{
                    marginTop: 6,
          ...
          >
            <button
              type="button"
              onClick={() => setConfiguralOpen((current) => !current)}
          ...
          <div className="grid" style={{ gridTemplateColumns: '1fr 1fr 1fr', gap: 8, paddingTop: 25 }}>
            {[
              { id: 'permutation-count', label: 'Permutations', value: permutationsInput, error: permutationsError, onChange: setPermutationsInput, inputMode: 'numeric' as const },
              { id: 'permutation-alpha', label: 'Alpha', value: alphaInput, error: alphaError, onChange: setAlphaInput, inputMode: 'decimal' as const },
              { id: 'permutation-seed', label: 'Seed', value: seedInput, error: seedError, onChange: setSeedInput, inputMode: 'numeric' as const },
            ].map((field) => (
