...
- `2026-07-04` — New behavior: the label behaves like a responsive text box. It wraps to fill up to 80% of the construct's width AND 80% of its height (`LABEL_BOX_FILL_RATIO = 0.8`), stays horizontally and vertically centred, and shrinks the font (down to `LABEL_MIN_FONT_SIZE = 6`) when the text cannot fit.
- `2026-07-04` — Wrapping rule (user requirement, revised same day): lines break ONLY at spaces or after hyphens ("Self-Esteem" → "Self-" / "Esteem"); a word is NEVER broken mid-word. When a whole word is wider than the box at the current font size, the font shrinks until it fits — an earlier draft that character-broke long words read awkwardly and was rejected by the user.
- `2026-07-04` — Curved-edge rule (user feedback, revised same day): a first draft used a plain 85% bounding box, which visually "covered the whole circle" because a rectangle 85% of the bounding box pokes outside a circle at its corners. Fix: ratio lowered to 80% AND for circles/ovals the wrapped block (widest line W × total height H) must satisfy the ellipse equation `(W/maxWidth)^2 + (H/maxHeight)^2 <= 1`, keeping block corners inside the curved edge while still allowing a short single line to use nearly the full 80% width. Rectangles use the plain `W <= maxWidth && H <= maxHeight` check.
- `2026-07-04` — Shape responsiveness: the box is `2*rx*0.8` wide by `2*ry*0.8` tall, so it is naturally wide for wide ovals and tall for tall rectangles; the wrap/shrink pass adapts line count to the available dimension. `resFontSize` (`max(9, min(rx,ry)*0.36)`) is kept as the ideal/maximum font size so short labels look unchanged.
- `2026-07-04` — Implementation in `ModelCanvas.tsx`: helpers `tokenizeLabel` (words + after-hyphen segments with a `glue` flag), `wrapLabelLines` (greedy wrap; returns `null` when an unbreakable token is too wide, signalling the shrink loop; `force` mode for the min-font best-effort fallback), `layoutConstructLabel` (font-size loop from `resFontSize` down to 6 with the rect/ellipse fit check), and `measureLabelWidth` (cached offscreen `<canvas>` 2D context, deterministic char-width fallback). Layouts memoized in `labelLayoutCache` keyed by text + box dims + font spec + shape kind (cache clears past 2000 entries). Rendering emits one `<tspan x=0 y=…>` per line with `dominantBaseline="central"`, vertically centred as `(li - (N-1)/2) * lineHeight`.
- `2026-07-04` — Scope note: the realtime R²-preview label branch (name + R² beneath) was intentionally left single-line to preserve its fixed two-line layout. Constants live near the other shape constants; `LABEL_LINE_HEIGHT_RATIO = 1.15`.
- `2026-07-04` — Verification: `npx tsc --noEmit` passed after both passes. Standalone Node sanity tests confirmed: no mid-word breaks in any case, hyphen wrapping at full font size, font shrink (not word breaking) when a word exceeds the box, ellipse fit ≤ 1 for circle/oval cases, correct rect behavior for tall/wide rectangles, and graceful min-font fallback for impossibly small shapes. A subagent code review of the first pass returned "Ready to merge: Yes" with only minor notes (clear-all cache eviction vs LRU, module-level cache state, best-effort overflow on tiny shapes) — accepted as-is. Browser preview was not run because the canvas requires a loaded workspace/model over Electron IPC.
